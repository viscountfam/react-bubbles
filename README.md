...
- [x] Explain what a token is used for.
      A token allows the server to verify that the user is authentic without the need for the input of the credentials that the user used to get the token.
- [x] What steps can you take in your web apps to keep your data secure?
      One practice that you can employ in order to protect your data in your web apps is to require users to authenticate themselves to ensure that they are authentic users by providing the server with credentials that the server can use to keep track of whose accessing the web app
- [x] Describe how web servers work.
      Web servers are computers that are connected directly to the internet which is a network of connected computers. Servers are where websites are hosted.When a person accesses the internet they do so by acessing a modem which then goes to the internet service provider and the goes to the DNS server that was called with the web address
